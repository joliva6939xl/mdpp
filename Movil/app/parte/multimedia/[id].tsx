...
export default function MultimediaScreen() {
  ...
  // La API ahora devuelve la ruta relativa completa (ej: "partes/123/archivo.jpg")
  // Solo necesitamos construir la URL base del servidor para los archivos.
  const baseMultimediaUrl = `${API_URL.replace("/api", "")}/uploads`;
          ...
          <View style={styles.fila}>
            {parte.fotos.map((f, i) => (
              <TouchableOpacity
                key={i}
                onPress={() => {
                  setFotoSeleccionada(`${baseMultimediaUrl}/${f}`);
                  setFotoVisible(true);
                }}
              >
                <Image
                  source={{ uri: `${baseMultimediaUrl}/${f}` }}
      ...
      {parte.videos && parte.videos.length > 0 && (
        <View>
          ...
          {parte.videos.map((v, i) => (
            <VideoPlayerComponent key={i} uri={`${baseMultimediaUrl}/${v}`} />
          ))}
        </View>
      )}
